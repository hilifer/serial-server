...
|---|---|---|---|
| **COM31** | 27 个车位检测器（A 区 1-27） | `ParkingCache`，每 2.0s 完整扫一遍 | ~2-3 秒 |
| **COM32** | 27 个车位检测器（B 区 28-54） | `ParkingCache`，每 2.0s 完整扫一遍 | ~2-3 秒 |
| **COM33** | 10 个电表（addr 1,2,3,4,5,6,8,9,10,11）<br>没有 7 | `MeterCache`，每 2.0s 完整扫一遍 | ~1-2 秒 |
| **COM33** | 同上 10 个电表 | `YearlyCache`，每 **3600s**（1 小时）扫一遍 12 个月冻结电量 | ~30-60 秒（每表间停 0.2s） |

实际节奏：
- COM31 / COM32 上每秒约发 **13-14 个请求**（27 个/2s）
- COM33 上 MeterCache 每秒约 **5-10 个请求**（10 个/2s，每个电表分多个寄存器组，所以请求数比电表多）
...
```yaml
cache:
  meter_interval: 2.0        # ↑ 调大降低 COM33 负载（实时数据更陈旧）
  parking_interval: 2.0      # ↑ 调大降低 COM31/32 负载
...
| 总线 | 利用率（估算） | 余量 |
|---|---|---|
| COM31 (27 车位/2s) | ~30% | 充足，可上 2-3 倍 |
| COM32 (27 车位/2s) | ~30% | 充足 |
| COM33 (10 电表/2s + 偶发 yearly) | 平时 ~25%，yearly 期间冲到 80%+ | 平时充足，整点峰值偏高 |

不建议把 `meter_interval`/`parking_interval` 降到 1.0 以下——一旦扫描时长超过 interval，缓存线程就开始排队、永远跟不上节奏，效果反而劣化。
