...
后端有一组后台守护线程不停往**串口（RS-485）**、**Modbus TCP**、**Odoo HTTP**这三类外设刷数据，把结果存内存。前端 HTTP 接口只读内存，不直接打外设。所有间隔都在 `config.yaml` 的 `cache:` 段配置，下面是当前默认值。

### 9.1 RS-485（三条独立 9600 baud 总线）

| 总线 | 挂的设备 | 后台扫描 | 单次扫描总时长（典型） |
|---|---|---|---|
| **COM31** | 27 个车位检测器（A 区 1-27） | `ParkingCache`，每 5.0s 完整扫一遍 | ~1-1.5 秒 |
| **COM32** | 27 个车位检测器（B 区 28-54） | `ParkingCache`，每 5.0s 完整扫一遍 | ~1-1.5 秒 |
| **COM33** | 10 个电表（addr 1,2,3,4,5,6,8,9,10,11）<br>没有 7 | `MeterCache`，每 5.0s 完整扫一遍 | ~1-2 秒 |
| **COM33** | 同上 10 个电表 | `YearlyCache`，每 **3600s**（1 小时）扫一遍 12 个月冻结电量 | ~30-60 秒（每表间停 0.2s） |

实际节奏：
- COM31 / COM32 上每秒约发 **5-6 个请求**（27 个/5s）
- COM33 上 MeterCache 每秒约 **2-4 个请求**（10 个/5s，每个电表分多个寄存器组）
- COM33 上 YearlyCache 平时不跑，**整点附近会突发 1 分钟左右的高密度扫描**，期间 MeterCache 会让出锁（短暂的实时数据延迟）
- 串口超时 `0.3s`，请求重试在底层做（CRC 错或无响应不会无限阻塞）

调节方式：改 `config.yaml`：

```yaml
cache:
  meter_interval: 5.0        # ↑ 调大降低 COM33 负载（实时数据更陈旧）
  parking_interval: 5.0      # ↑ 调大降低 COM31/32 负载
  yearly_refresh: 3600.0     # ↑ 调大降低 COM33 突发占用
```
...
| 设备 | 地址 | 访问频率 | 备注 |
|---|---|---|---|
| 电池管理系统 (BMS) | `10.6.120.10:502` (config 可改) | **按需**，5 秒 TTL 缓存 | 只读 1 个寄存器（SOC %） |

行为：`/battery/soc` 被调时检查上次读取时间，超过 5 秒才发新请求；不到 5 秒直接返回内存值。前端就算每 1 秒调一次，对 BMS 的真实压力是**每 5 秒 1 次**。
...
### 9.3 Odoo HTTP（充电桩 / 充电枪后台）

| URL | 频率 | 方式 |
|---|---|---|
| `http://yaorongiot.cn:8069/web/dataset/call_kw` | **按需**，5 秒 TTL 缓存 | JSON-RPC |
| `http://yaorongiot.cn:8069/web/session/authenticate` | 每次会话过期重新登录 | 自动维持 session_id cookie |

行为：`/charging/piles` 和 `/charging/guns` 各自维护独立的 5 秒缓存（按调用参数为 key）。前端浏览器并发调多个，**对 Odoo 仍只产生 1 次每 5 秒**（每个不同的请求体）。
...
| 外设 | 不通时表现 |
|---|---|
| RS-485（COM 口） | `SerialManager` 后台自动重连；后台线程每秒检查一次端口，端口未恢复就跳过该轮、保留上次值 |
| BMS Modbus TCP | `/battery/soc` 返回 `503`；下次 5 秒到期再试 |
| Odoo HTTP | 缓存软回退——返回**上次成功的值**；服务端日志会出现 `Odoo call failed: ...` warning，但前端不会拿到空响应 |

### 9.5 总线占用上限

每条 RS-485 总线（9600 baud）理论吞吐 ~960 字节/秒，扣掉应答、间隔，**实际可用约 30-50 个 Modbus 请求/秒**。当前配置下：

| 总线 | 利用率（估算） | 余量 |
|---|---|---|
| COM31 (27 车位/5s) | ~12% | 大量余量 |
| COM32 (27 车位/5s) | ~12% | 大量余量 |
| COM33 (10 电表/5s + 偶发 yearly) | 平时 ~10%，yearly 期间冲到 80%+ | 平时大量余量，整点峰值偏高 |

不建议把 `meter_interval`/`parking_interval` 降到 1.0 以下——一旦扫描时长超过 interval，缓存线程就开始排队、永远跟不上节奏，效果反而劣化。当前 5.0 秒间隔留出充足余量。
